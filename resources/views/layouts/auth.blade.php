...
<body
    ...
    <div class="flex min-h-screen w-full">
        <div class="relative hidden flex-1 flex-col justify-between overflow-hidden border-r border-gray-200 bg-gradient-to-br from-brand-500 via-brand-600 to-brand-700 px-10 py-12 text-white dark:border-gray-800 dark:from-gray-900 dark:via-gray-900 dark:to-black lg:flex">
            <div class="flex items-center justify-between">
                <a href="{{ Route::has('dashboard') ? route('dashboard') : '#' }}" class="flex items-center gap-3">
                    <img src="{{ asset('assets/tailadmin/images/logo/logo-icon.svg') }}" class="h-10 w-10"
                        alt="Logo icon">
                    <span class="text-lg font-semibold tracking-wide lg:text-xl">{{ config('app.name', 'TailAdmin') }}</span>
                </a>
                <div class="hidden rounded-full border border-white/30 px-4 py-1.5 text-sm/6 font-medium tracking-wide lg:flex">
                    @yield('badge', 'Admin Portal')
                </div>
            </div>
        ...
        <main class="flex w-full flex-1 items-center justify-center px-6 py-10 sm:px-8 lg:px-16">
            <div class="w-full max-w-md space-y-8">
                <div class="flex items-center gap-3 text-brand-600 dark:text-brand-400 lg:hidden">
                    <img src="{{ asset('assets/tailadmin/images/logo/logo-icon.svg') }}" class="h-10 w-10" alt="Logo icon">
