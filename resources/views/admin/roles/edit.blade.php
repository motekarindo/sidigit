        ...
        <div class="flex items-baseline justify-between gap-3">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Sesuaikan detail peran serta hak akses yang dimiliki.
                </p>
            </div>
            ...
            <form action="{{ route('roles.update', $role) }}" method="POST" class="space-y-8">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">Pilih izin yang diperbolehkan untuk peran ini.
                        </p>
                    </div>
                    ...
                    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                        @foreach ($menus as $menu)
                            <div
                                class="flex h-full flex-col rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-950/50">
                                <div
                                    class="border-b border-gray-100 px-4 py-3 text-sm font-semibold text-gray-800 dark:border-gray-900 dark:text-white">
                                    {{ $menu->name }}
                                </div>
                                <div class="flex-1 space-y-4 px-4 py-4 text-sm text-gray-700 dark:text-gray-300">
                                        ...
                                        @foreach ($menu->children->sortBy('order') as $child)
                                            <div class="space-y-2">
                                                ...
                                                <div class="space-y-2 rounded-xl bg-gray-50 p-3 dark:bg-gray-900/60">
                                                    ...
                                                    @empty
                                                        <p class="text-xs text-gray-400 dark:text-gray-500">Tidak ada
                                                            permission</p>
                                                    @endforelse
                                                </div>
                                            </div>
                                    ...
                                    @else
                                        <div class="space-y-2">
                                            ...
                                            @empty
                                                <p class="text-xs text-gray-400 dark:text-gray-500">Tidak ada permission</p>
                                            @endforelse
                                        </div>
                                    @endif
                                </div>
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            Menu yang dicentang akan terlihat di sidebar untuk pengguna dengan role ini.
                        </p>
                    </div>
                            ...
                            <div class="rounded-2xl border border-gray-200 p-4 dark:border-gray-800 dark:bg-gray-950/50">
                                <label class="flex items-start gap-3 text-sm font-semibold text-gray-900 dark:text-white">
                                    <input type="checkbox" name="menus[]" value="{{ $menu->id }}"
                                        ...
                                        @checked(in_array($menu->id, $roleMenus))>
                                    {{ $menu->name }}
                                </label>
                                @if ($menu->children->isNotEmpty())
                                    <div
                                        class="mt-3 space-y-2 border-l border-dashed border-gray-200 pl-4 dark:border-gray-700">
                                        @foreach ($menu->children->sortBy('order') as $child)
                                            <label class="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-300">
                                                <input type="checkbox" name="menus[]" value="{{ $child->id }}"
                                                    id="menu-{{ $child->id }}"
                                                    class="size-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500/40 dark:border-gray-700 dark:bg-gray-900"
                                                    @checked(in_array($child->id, $roleMenus))>
                                                {{ $child->name }}
                                            </label>
                                        @endforeach
                                    </div>
