...
@section('content')
    <div class="space-y-6">
        <div class="flex items-baseline justify-between gap-3">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Edit Permission: {{ $permission->name }}</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Sesuaikan nama, slug, atau menu induk untuk izin ini.
                </p>
            </div>
                ...
                <div>
                    ...
                    <select id="menu_id" name="menu_id"
                        ...
                        @foreach ($menus as $menu)
                            <option value="{{ $menu->id }}" @selected(old('menu_id', $permission->menu_id) == $menu->id)>
                                {{ $menu->name }}
                            </option>
                        @endforeach
                    </select>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        Mengelompokkan permission berdasarkan menu memudahkan pencarian dan manajemen.
                    </p>
                    @error('menu_id')
                        <p class="mt-1 text-sm text-error-500 dark:text-error-300">{{ $message }}</p>
                    @enderror
