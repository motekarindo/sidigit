...
@section('title', 'Buat Transaksi Order')

@section('content')
    <div class="space-y-6">
        <div class="flex items-baseline justify-between gap-3">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Buat Transaksi Order</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Catat pesanan baru, atur item berdasarkan produk, dan tentukan pembayaran lunas atau cicilan.
                </p>
            </div>
            <a href="{{ route('transaction-orders.index') }}"
        ...
        <div class="space-y-8 rounded-2xl border border-gray-200 bg-white shadow-theme-sm dark:border-gray-800 dark:bg-gray-950/70">
            @if (session('error'))
                <div
            ...
            <form action="{{ route('transaction-orders.store') }}" method="POST" class="space-y-4" id="transaction-order-form">
                @csrf

                @include('admin.transaction-order._form', [
                    'order' => null,
                    'customers' => $customers,
                    'units' => $units,
                    'products' => $products,
