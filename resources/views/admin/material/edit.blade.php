...
@section('content')
    <div class="space-y-6">
        <div class="flex items-baseline justify-between gap-3">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Edit Material</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Perbarui informasi material sesuai kebutuhan.
                </p>
            </div>
        ...
        <div class="space-y-8 rounded-3xl border border-gray-200 bg-white p-8 shadow-theme-sm dark:border-gray-800 dark:bg-gray-950/70">
            ...
            <form action="{{ route('materials.update', $material) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')

                @include('admin.material._form', ['material' => $material, 'categories' => $categories, 'units' => $units])
