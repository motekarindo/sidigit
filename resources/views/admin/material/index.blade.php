...
@section('content')
    ...
    <div class="space-y-6">
        <div class="flex items-baseline justify-between gap-3">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Daftar Material</h1>
                        ...
                        @forelse ($materials as $material)
                            <tr class="hover:bg-gray-50/60 dark:hover:bg-gray-900/50">
                                <td class="px-4 py-3 text-center text-gray-600 dark:text-gray-400">
                                    {{ $loop->iteration + $materials->firstItem() - 1 }}
                                </td>
                                ...
                                <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">
                                    {{ number_format($material->reorder_level, 2, ',', '.') }}
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">
                                    {{ $material->description ? Str::limit($material->description, 60) : '–' }}
                                </td>
                                <td class="px-4 py-3">
                                        ...
                                        @can('material.delete')
                                            <div x-data="{ isConfirmOpen: false }" @keydown.escape.window="isConfirmOpen = false">
                                                ...
                                                <div x-cloak x-show="isConfirmOpen" x-transition.opacity
                                                    ...
                                                    role="dialog" aria-modal="true" aria-labelledby="deleteMaterialTitle{{ $material->id }}"
                                                    ...
                                                    <div x-show="isConfirmOpen" x-transition.scale
                                                        ...
                                                        <div class="flex items-start gap-3">
                                                            ...
                                                            <div class="space-y-2">
                                                                <h3 id="deleteMaterialTitle{{ $material->id }}"
                                                                    class="text-lg font-semibold text-gray-900 dark:text-white">
                                                                    Hapus Material?
                                                                </h3>
                                                                <p class="text-sm text-gray-600 dark:text-gray-400">
                                                                    Tindakan ini akan menghapus material
                                                                    <span class="font-semibold text-gray-900 dark:text-white">{{ $material->name }}</span>
                                                                    secara permanen.
                                                                </p>
                                                            </div>
                                                        </div>
