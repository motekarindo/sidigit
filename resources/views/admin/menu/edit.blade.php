...
@section('content')
    <div class="space-y-6">
        <div class="flex items-baseline justify-between gap-3">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Edit Menu: {{ $menu->name }}</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Perbarui informasi menu untuk menjaga navigasi tetap terstruktur.
                </p>
            </div>
        ...
        <div class="rounded-3xl border border-gray-200 bg-white p-8 shadow-theme-sm dark:border-gray-800 dark:bg-gray-950/70">
            <form action="{{ route('menus.update', $menu) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                Kosongkan jika menu ini hanya sebagai induk tanpa route.
                            </p>
                            @error('route_name')
                                <p class="mt-1 text-sm text-error-500 dark:text-error-300">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select id="icon" name="icon"
                                ...
                                @foreach ($icons as $icon)
                                    <option value="{{ $icon }}" @selected(old('icon', $menu->icon) == $icon)>
                                        {{ $icon }}
                                    </option>
                                @endforeach
                            </select>
