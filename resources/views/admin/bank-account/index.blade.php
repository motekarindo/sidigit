...
@section('content')
    <div class="space-y-6">
        <div class="flex items-baseline justify-between gap-3">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Daftar Rekening Bank</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Kelola daftar rekening perusahaan untuk kebutuhan transaksi.
                </p>
            </div>
        ...
        <div class="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-theme-sm dark:border-gray-800 dark:bg-gray-950/70">
                        ...
                        @forelse ($bankAccounts as $bankAccount)
                            <tr class="hover:bg-gray-50/60 dark:hover:bg-gray-900/50">
                                <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">
                                    {{ $loop->iteration + $bankAccounts->firstItem() - 1 }}
                                </td>
                                <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">
                                    {{ $bankAccount->rekening_number }}
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">
                                    ...
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <div x-data="{ isConfirmOpen: false }" @keydown.escape.window="isConfirmOpen = false">
                                            ...
                                            <div x-cloak x-show="isConfirmOpen" x-transition.opacity
                                                ...
                                                role="dialog" aria-modal="true" aria-labelledby="deleteBankAccountTitle{{ $bankAccount->id }}"
                                                ...
                                                <div x-show="isConfirmOpen" x-transition.scale
                                                    ...
                                                    <div class="flex items-start gap-3">
                                                        ...
                                                        <div class="space-y-2">
                                                            <h3 id="deleteBankAccountTitle{{ $bankAccount->id }}"
                                                                class="text-lg font-semibold text-gray-900 dark:text-white">
                                                                Hapus Rekening?
                                                            </h3>
                                                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                                                Tindakan ini akan menghapus rekening
                                                                <span class="font-semibold text-gray-900 dark:text-white">{{ $bankAccount->account_name }}</span>
                                                                dari daftar. Proses ini tidak dapat dibatalkan.
                                                            </p>
                                                        </div>
                                                    </div>
            ...
            <div class="border-t border-gray-100 px-4 py-4 dark:border-gray-900">
                {{ $bankAccounts->links('components.pagination') }}
            </div>
        </div>
    </div>
@endsection
