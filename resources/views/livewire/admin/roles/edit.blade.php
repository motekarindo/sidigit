<div class="space-y-2">
    ...
    <x-card class="space-y-8 rounded-3xl p-8 shadow-theme-sm">
        <form wire:submit.prevent="update" class="space-y-8">
            ...
            <div class="space-y-4" wire:init="loadMenuOptions">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div class="flex flex-wrap items-center gap-4 text-sm text-gray-600 dark:text-gray-300">
                        <label class="inline-flex items-center gap-2">
                            <input type="checkbox" wire:model="selectAllMenus" @disabled(! $menuOptionsReady)
                                class="size-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500/40 dark:border-gray-700 dark:bg-gray-900">
                            <span>Semua Menu</span>
                        </label>
                        <label class="inline-flex items-center gap-2">
                            <input type="checkbox" wire:model="selectAllPermissions" @disabled(! $menuOptionsReady)
                                class="size-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500/40 dark:border-gray-700 dark:bg-gray-900">
                ...
                @if ($menuOptions->isEmpty())
                    <div class="rounded-2xl border border-dashed border-gray-200 bg-white p-6 text-sm text-gray-500 dark:border-gray-800 dark:bg-gray-950/60 dark:text-gray-400">
                        Memuat daftar menu...
                    </div>
                @else
                                        ...
                                        @forelse ($menuPerms as $permission)
                                            <label class="flex items-center gap-3 text-sm text-gray-700 dark:text-gray-300">
                                                <input type="checkbox" value="{{ $permission['id'] }}" wire:model="permissions"
                                                    @checked(in_array($permission['id'], $permissions, true))
                                                    class="size-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500/40 dark:border-gray-700 dark:bg-gray-900">
