<div class="space-y-6">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Kelola akun pengguna beserta perannya.</p>
        </div>
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div class="relative">
                <input type="search" wire:model.debounce.400ms="search" placeholder="Cari nama, email, atau username"
            ...
            @can('users.create')
                <a href="{{ route('users.create') }}"
                    class="inline-flex items-center justify-center gap-2 rounded-2xl bg-brand-500 px-4 py-2 text-sm font-semibold text-white shadow-theme-sm transition hover:bg-brand-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2 dark:ring-offset-gray-900">
                    Tambah User
                </a>
            @endcan
        </div>
    ...
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-sm dark:border-gray-800 dark:bg-gray-950/70">
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-800">
                ...
                <tbody class="divide-y divide-gray-100 bg-white dark:divide-gray-900 dark:bg-gray-950/40">
                    @forelse ($users as $user)
                        <tr class="hover:bg-gray-50/60 dark:hover:bg-gray-900/40" wire:key="user-{{ $user->id }}">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
                                <div class="flex flex-wrap gap-1">
                                    @forelse ($user->roles as $role)
                                        <span class="rounded-full bg-brand-50 px-2 py-0.5 text-xs font-semibold text-brand-600 dark:bg-brand-500/10 dark:text-brand-300">
                                            {{ $role->name }}
                                        </span>
                                    @empty
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    @can('users.edit')
                                        <a href="{{ route('users.edit', $user) }}"
                                            class="inline-flex items-center rounded-xl border border-gray-200 px-3 py-2 text-xs font-semibold text-gray-600 transition hover:border-brand-200 hover:text-brand-600 dark:border-gray-700 dark:text-gray-300 dark:hover:border-brand-500 dark:hover:text-brand-400">
                                            Edit
                                        </a>
                                    @endcan
                                    @can('users.delete')
                                        <button type="button"
                                            x-data="{}"
                                            x-on:click.prevent="if (confirm('Hapus user {{ $user->name }}?')) $wire.deleteUser({{ $user->id }});"
                                            class="inline-flex items-center rounded-xl border border-error-200 bg-error-50 px-3 py-2 text-xs font-semibold text-error-600 transition hover:border-error-300 hover:bg-error-100 dark:border-error-500/40 dark:bg-error-500/10 dark:text-error-200 dark:hover:bg-error-500/20">
                                            Hapus
                                        </button>
                                    @endcan
                                </div>
