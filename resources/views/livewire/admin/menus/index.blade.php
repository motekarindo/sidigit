<div class="space-y-6">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Atur struktur navigasi aplikasi dan urutan tampil.</p>
        </div>
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div class="relative">
                <input type="search" wire:model.debounce.400ms="search" placeholder="Cari nama atau route"
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-800">
                <thead class="bg-gray-50 dark:bg-gray-900/50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Parent</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Route</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Urutan</th>
                ...
                <tbody class="divide-y divide-gray-100 bg-white dark:divide-gray-900 dark:bg-gray-950/40">
                    @forelse ($menus as $menu)
                        <tr class="hover:bg-gray-50/60 dark:hover:bg-gray-900/50" wire:key="menu-{{ $menu->id }}">
                            <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $menu->name }}</td>
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{{ $menu->parent?->name ?? '-' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{{ $menu->route_name ?? '-' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{{ $menu->order }}</td>
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    @can('menu.delete')
                                        <button type="button"
                                            x-data="{}"
                                            x-on:click.prevent="if (confirm('Hapus menu {{ $menu->name }}?')) $wire.deleteMenu({{ $menu->id }});"
