...
class RolePermissionSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if ($ownerRole) {
            // Owner (ditunjuk superadmin) dapat semua menu operasional klien
            $ownerRole->menus()->sync($allMenus);
        }

        if ($kasirRole) {
            // User kasir dapat menu tertentu
            $kasirMenu = Menu::whereIn('name', [
                ...
                'Laporan Per Cabang',
                'Master', 'Supplier', 'Gudang', 'Finishing'
            ])->pluck('id');
        ...
        if ($ownerRole) {
            // Owner mendapat hak tertinggi pada tenant/client (tanpa akun superadmin)
